...
func applyAction(referencePoint coordinate, action string) (path []coordinate, lastCoordinate coordinate) {
	// Parse the action
	direction := action[0]
	movement, _ := strconv.Atoi(action[1:])
	lastCoordinate = referencePoint

	switch direction {
	case 'R':
		for i := 1; i <= movement; i++ {
			var coord coordinate
			coord.x = referencePoint.x + i
			coord.y = referencePoint.y
			coord.steps = lastCoordinate.steps + 1
	...
	case 'U':
		for i := 1; i <= movement; i++ {
			var coord coordinate
			coord.x = referencePoint.x
			coord.y = referencePoint.y + i
			coord.steps = lastCoordinate.steps + 1
			path = append(path, coord)
			lastCoordinate = coord
		}
	...
	fmt.Printf("Coord steps:%d\n", lastCoordinate.steps)
	return path, lastCoordinate
}

// return the full coordinates path from the centralPort
// from the set of actions
func getFullPathFromActions(centralPort coordinate, actions []string) []coordinate {
	var path []coordinate
...
// return the set of common coordinates for the two sets passed by
func getCommonCoordinates(set1 []coordinate, set2 []coordinate) (coordinates []coordinate) {
	for _, coord1 := range set1 {
		for _, coord2 := range set2 {
			if coord1.x == coord2.x &&
				coord1.y == coord2.y {
				// Add both coordinates steps to the final coordinate
				finalCoord := coord1
				finalCoord.steps = coord1.steps + coord2.steps
				fmt.Printf("Final coord: %d\n", finalCoord.steps)
				coordinates = append(coordinates, finalCoord)
			}
		}
	}
	return
...
func getClosestCommonCoordinate(referencePoint coordinate, coordinates []coordinate) (closestCommonCoordinateDistance int) {
	var distance int = 0

	for _, c := range coordinates {
		xCalc := absValue(c.x - referencePoint.x)
		yCalc := absValue(c.y - referencePoint.y)

		currentDistance := xCalc + yCalc
